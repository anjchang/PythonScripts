...
class VizGen:
    ...
    def generate_location_vis(self):
        ...
        for row in c:
            ...
            if int(data_dict["TIMESTAMP"]) - last_date >= 60:
                location_data.append([data_dict['LOCATION']['mLongitude'], data_dict['LOCATION']['mLatitude']])
                last_date = int(data_dict["TIMESTAMP"])
            self.movement_location_data.append(LocTuple(data_dict['LOCATION']['mLongitude'], data_dict['LOCATION']['mLatitude'], datetime.fromtimestamp(float(data_dict["TIMESTAMP"]))))
    ...
    def generate_activity_vis(self):
        ...
        for row in c:
            ...
            #Because of probe updates
            if 'HIGH_ACTIVITY_INTERVALS' in data_dict:
                activity_list.append([data_dict['TIMESTAMP'], data_dict['TOTAL_INTERVALS'], data_dict["HIGH_ACTIVITY_INTERVALS"]])
            else:
                activity_list.append([data_dict['TIMESTAMP'] * 1000, data_dict['TOTAL_INTERVALS'], data_dict["ACTIVE_INTERVALS"]])
    ...
    def output_page(self):
        ...
        fordDict = {'genDate': today.strftime("%B %d, %Y %I:%M%p"), #'sleepRanges': self.sleep_ranges,
                    ...
                    'totalScans': str(self.locationScans + self.batteryScans + self.activityScans),
